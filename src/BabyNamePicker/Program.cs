...
api.MapGet("/popularity", async (int year, string? gender, NameQueryService service, CancellationToken ct) =>
    Results.Ok(await service.GetPopularityAsync(year, gender, ct)));
...
static async Task InitializeDatabaseAsync(WebApplication app)
{
    ...
    if (!await db.Names.AnyAsync())
    {
        ...
        logger.LogInformation("Database empty; seeding SSA data...");
        await importer.ImportAsync(new SsaImportOptions());
    }
